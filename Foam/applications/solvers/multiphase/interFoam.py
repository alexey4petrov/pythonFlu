...
def alphaEqnSubCycle( runTime, piso, mesh, phi, alpha1, rho, rhoPhi, rho1, rho2, interface ):
    ...
    nAlphaCorr = readInt( piso.lookup( word( "nAlphaCorr" ) ) )
    nAlphaSubCycles = readInt( piso.lookup( word( "nAlphaSubCycles" ) ) )
...
if WM_PROJECT_VERSION() >= "1.6" :
   if __name__ == "__main__" :
      ...
      if len( argv ) > 1 and argv[ 1 ] == "-test":
         argv = None
         test_dir= os.path.join( os.environ[ "PYFOAM_TESTING_DIR" ],'cases', 'r1.6', 'basic', 'interFoam', 'flange' )
